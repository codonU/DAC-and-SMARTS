...
class OptionCriticNet(nn.Module, BaseNet):
    ...
    q:     [feature_dim, num_options] 选择option

    pi:    [feature_dim, num_options, action_dim] 选择option中的action 

    beta:  [feature_dim, num_options] 输出终止option的概率
    """
    ...
    def forward(self, x):
        ...
        pi = self.fc_pi(phi)
        pi = pi.view(-1, self.num_options, self.action_dim) # 改变结构
        log_pi = F.log_softmax(pi, dim=-1)  # log_softmax = log(softmax)
...
class OptionGaussianActorCriticNet(nn.Module, BaseNet):
    ...
    def forward(self, obs):
        ...
        return {'mean': mean,                   # [1, 4, 6]
                'std': std,                     # [1, 4, 6]
                'q_o': q_o,                     # [1, 4]
                'u_o': u_o,                     # [1, 5]
                'inter_pi': pi_o,               # [1, 4]
                'log_inter_pi': log_pi_o,       # [1, 4]
                'beta': beta                    # [1, 4]
            }                   

class FixedSTDOptionNet(nn.Module):
    def __init__(self,
                 action_dim,
                 body_fn):
        super(DeterministicOptionNet, self).__init__()
        self.pi_body = body_fn()
        self.beta_body = body_fn()
        self.fc_pi = layer_init(nn.Linear(self.pi_body.feature_dim, action_dim), 1e-3)
        self.fc_beta = layer_init(nn.Linear(self.beta_body.feature_dim, 1), 1e-3)
        # self.std = nn.Parameter(torch.zeros((1, action_dim)))
        self.std = 0.5
        self.std = torch.zeros((1, action_dim)) + self.std
...
class FixedSTDOptionCritic(nn.Module, BaseNet):
    def __init__(self,
                state_dim,
                action_dim,
                num_options,
                ...
                critic_body=None,
                option_body_fn=None):
        super(DeterministicOptionCritic, self).__init__()
        if phi_body is None: phi_body = DummyBody(state_dim)
        if critic_body is None: critic_body = DummyBody(phi_body.feature_dim)
        if actor_body is None: actor_body = DummyBody(phi_body.feature_dim)
        ...
        self.options = nn.ModuleList([FixSTDOptionNet(action_dim, option_body_fn) for _ in range(num_options)])
