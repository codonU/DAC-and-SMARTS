...
class ASquaredCPPOAgent(BaseAgent):
    ...
    def compute_pi_hat(self, prediction, prev_option, is_intial_states):
        ...
        计算MDP—H的\pi
        """
        inter_pi = prediction['inter_pi']       # \pi_o
        mask = torch.zeros_like(inter_pi)
        mask[self.worker_index, prev_option] = 1
        beta = prediction['beta']
        # mask:是不是还是上次的option
        pi_hat = (1 - beta) * mask + beta * inter_pi
        is_intial_states = is_intial_states.view(-1, 1).expand(-1, inter_pi.size(1))    # -1是自动适配！

        # 论文里的 \pi^\mathbb{H}
        # 结合torch.where(condition, x, y) 满足条件为x, 不满足为y
        # inter_pi: 网络输出，为\pi_o, 上层策略选择option的概率
        # pi_hat: 之前计算的转移概率， P(O_t|O_t-1, S_t)
    ...
    def compute_pi_bar(self, options, action, mean, std):
        ...
        也就是option的intra-policy
        """
        # unsqueeze(-1): 在最后插一个维度……
    ...
    def step(self):
        ...
        for _ in range(config.rollout_length):
            ...
            # 计算上层策略选择option
            pi_hat = self.compute_pi_hat(prediction, self.prev_options, self.is_initial_states)
            dist = torch.distributions.Categorical(probs=pi_hat)    # categorical 确定性的
            options = dist.sample()
            ...
            # 根据option选择action
            # 但是worker_index 为 np.arange(num_worker)
            # 没有太懂
            # num_worker默认为1， 没有多任务, self.worker_index = [0]
            # 所以都是1行 
